...
def attack(fuzz_val,allowed_qrs,top_k_words, text_ls, true_label, predictor, stop_words_set, word2idx, idx2word, cos_sim, sim_predictor=None,
    ...
    if true_label != orig_label:
        ...
        for idx, synonyms in synonyms_all:
            ...
            if fuzz.token_set_ratio(' '.join(text_ls) , ' '.join(text_cache)) < fuzz_val:
                return ' '.join(text_prime), num_changed, orig_label, torch.argmax(predictor([text_prime])), num_queries, words_perturb
...
def main():
    ...
    success=[]
    results=[]
    log_file = open('logr_orignal.txt', 'a')
    ...
    for idx, (text, true_label) in enumerate(data):
        ...
        if args.perturb_ratio > 0.:
            ...
            new_text, num_changed, orig_label, \
            new_label, num_queries,pwrds = attack(args.fuzz,args.allowed_qrs,args.top_k_words,
    ...
    message = 'For target model {} allowed qrs {} top words {}: original accuracy: {:.3f}%, adv accuracy: {:.3f}%, ' \
    ...
    with open('results11.csv', 'w') as csvfile: 
