...
def attack(sample_index,premise, hypothese, true_label, predictor, stop_words_set, word2idx, idx2word, cos_sim, sim_predictor=None,
           import_score_threshold=-1., sim_score_threshold=0.5, sim_score_window=15, synonym_num=50, batch_size=32):
    # first check the prediction of the original text
    orig_probs = predictor({'premises': [premise], 'hypotheses': [hypothese]}).squeeze() #predictor(premise,hypothese).squeeze()
    orig_label = torch.argmax(orig_probs)
    orig_prob = orig_probs.max()
    rows=[]
    with open('./attention_weights/attend-snli.csv', 'r') as csvfile: 
    ...
    if true_label != orig_label:
        ...
        for idx in range(len(hypothese)):
            word_map[hypothese[idx]] = idx
        ...
        for wrd in rows[sample_index]:
            #print(wrd)
            indx = wrd.find(" ")
            final_word = wrd[2:indx-2]
            #if final_word not in stop_words_set:
            #print(final_word)
            words_perturb.append((word_map[final_word],final_word))
            #print(sample_index)
            #print(wrd[0])
        #print(words_perturb)
        # find synonyms
        words_perturb = words_perturb[:1]
        words_perturb_idx = [word2idx[word] for idx, word in words_perturb if word in word2idx]
        #synonym_words, synonym_values = pick_most_similar_words_batch(words_perturb_idx, cos_sim, idx2word, synonym_num, 0.5)
        ...
        for idx, word in words_perturb:
            if word in word2idx:
                ...
                vals=synonym_values.pop(0)
                #if all(vals[i] <= vals[i+1] for i in range(len(vals)-1)):
                #print(vals)
                if synonyms:
                    synonyms_all.append((idx, synonyms,vals))

        # start replacing and attacking
        text_prime = hypothese[:]
        text_cache = text_prime[:]
        num_changed = 0
        for idx, synonyms,vals in synonyms_all:
            ...
            new_probs = predictor({'premises': [premise] * len(index), 'hypotheses': new_texts1}) #predictor([premise] * len(index), [new_texts1])
            #print(new_probs)
            if len(new_probs.shape) < 2:
                new_probs = new_probs.unsqueeze(0)
            #breakpoint()
            #text_prime[idx] = synonyms[index]
            ...
            if np.sum(pr)>0 :
               ...
               final_index = (torch.argmin(new_probs[:,orig_label])).data.cpu().numpy()
               text_prime[idx] = synonyms[index[final_index][1]]
...
#                    num_changed += 1
            ...
            #    #print((new_probs_mask * semantic_sims))
            #    #print("####")
            #    replacement.append((new_probs_mask * semantic_sims).argmax())
            #    ind = ((new_probs_mask * semantic_sims).argmax())
            #    breakpoint()
            #    #print("["+str((new_probs_mask*semantic_sims).argmax())+" "+str(len(synonyms))+"]")
            #    num_changed += 1
            #    break
            #else:
            #   new_label_probs = new_probs[:, orig_label] + torch.from_numpy(
            ...
            #    if new_label_prob_min < orig_prob:
            #        text_prime[idx] = synonyms[new_label_prob_argmin]
            #        replacement.append(new_label_prob_argmin)
            #        breakpoint()
            #        #print("["+str(new_label_prob_argmin.data.cpu().numpy())+" "+str(len(synonyms))+"]",end=" ")
            #        num_changed += 1
            #text_cache = text_prime[:]
        if fuzz.token_set_ratio(' '.join(hypothese),' '.join(text_cache)) < 60 :
            return ' '.join(text_prime), num_changed, orig_label,torch.argmax(predictor({'premises':[premise], 'hypotheses': [text_prime]})), num_queries,words_perturb,[]
        return ' '.join(text_prime), num_changed, orig_label,torch.argmax(predictor({'premises':[premise], 'hypotheses': [text_prime]})), num_queries, words_perturb,[]
...
def main():
    ...
    with open("results-infersent-snli.csv","r") as csvfile:
        csvreader = csv.reader(csvfile)
    ...
    for idx, premise in enumerate(data['premises']):
        ...
        for r in rows:
            if r[0] == str(idx):
                hypo = r[2].split()
                for h in hypo:
                    if h not in stop_words_set and len(h)>2:
                        hypothese.append(h)
                flag=1
                tot+=1
                break
        if flag == 0:
            continue
        if idx % 20 == 0:
        #    up = open('up_imdb.txt','a')
        #    up.write("Samples Completed: "+str(idx))
        #    up.write("\n")
        #    up.write("Samples Attacked: "+str(len(success)))
        #    up.write("\n")
        #    up.write("Orignal Failures: "+str(orig_failures))
        #    up.write("\n")
        #    up.close()
        ...
        if args.perturb_ratio > 0.:
            ...
            orig_label  = attack(idx,premise, hypothese, true_label, predictor, stop_words_set,
        ...
        if true_label != orig_label:
            orig_failures += 1
        #wrds.append((true_label,orig_label))
    print(orig_failures)
    print("Acc: {:.3f}%".format((1-orig_failures/tot)*100))
