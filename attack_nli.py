...
def attack(sample_index,premise, hypothese, true_label, predictor, stop_words_set, word2idx, idx2word, cos_sim, word_embedding, sim_predictor=None,
    ...
    text_ls = text[:]
    orig_probs = predictor({'premises': [premise], 'hypotheses': [text_ls]}).squeeze() #predictor(premise,hypothese).squeeze()
    ...
    if true_label != orig_label:
        ...
        for idx, score in words_perturb:
            ...
            new_probs = predictor({'premises': [premise] * len(new_texts1), 'hypotheses': new_texts1})
        ...
        return ' '.join(text_prime), num_changed, orig_label, torch.argmax(predictor({'premises':[premise], 'hypotheses': [text_prime]})), num_queries, words_perturb,sims
...
def main():
    ...
    for idx, premise in enumerate(data['premises']):
        ...
        if args.perturb_ratio > 0.:
            ...
            new_text, num_changed, orig_label, \
            new_label, num_queries,pwrds,sims = attack(idx,premise, hypothese, true_label, predictor, stop_words_set,
        ...
        if true_label == orig_label and true_label != new_label:
            ...
            temp.append(changed_rate)
            temp.append(num_queries)
            results.append(temp)
            print(idx)

    with open('results-infersent-snli.csv', 'w') as csvfile: 
